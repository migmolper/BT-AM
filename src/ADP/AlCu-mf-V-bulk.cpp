/**
 * @file AlCu-mf-V-bulk.cpp
 * @author Miguel Molinos ([migmolper](https://github.com/migmolper))
 * @brief Compute meanfield-variables for the Al-Cu bulk
 * @version 0.1
 * @date 2023-05-19
 *
...
#include <mpi.h>
#endif
#include "ADP/AlCu-mf-V-bulk.hpp"
...
dmd_equations DMD_AlCu_constructor() {
  ...
  dmd_equations AlCu_dmd_equations;

  //!
  AlCu_dmd_equations.evaluate_rho_i = evaluate_rho_i_adp_AlCu;

  AlCu_dmd_equations.evaluate_DV_i_Dq_j = evaluate_DV_i_Dq_u_adp_AlCu;

  AlCu_dmd_equations.evaluate_D2V_i_Dq2_j = evaluate_D2V_i_Dq2_u_AlCu;

  AlCu_dmd_equations.evaluate_DV_i_DF = evaluate_DV_i_dF_AlCu;

  //! <V>0 set of equation
  AlCu_dmd_equations.evaluate_mf_rho_i = evaluate_mf_rho_i_adp_AlCu;

  AlCu_dmd_equations.evaluate_V0_i = evaluate_V0_i_adp_AlCu;

  AlCu_dmd_equations.evaluate_S0_i = evaluate_S0_i_adp_AlCu;

  AlCu_dmd_equations.evaluate_DV0_i_Dmeanq_j = evaluate_DV0_i_Dmeanq_u_AlCu;

  AlCu_dmd_equations.evaluate_D2V0_i_Dmeanq2_j = evaluate_D2V0_i_Dmeanq2_u_AlCu;

  AlCu_dmd_equations.evaluate_DV0_i_Dstdvq_j = evaluate_DV0_i_Dstdvq_u_AlCu;

  AlCu_dmd_equations.evaluate_DV0_i_Dq_j = evaluate_DV0_i_Dq_u_AlCu;

  AlCu_dmd_equations.evaluate_DV0_i_Dxi_j = evaluate_DV0_i_Dxi_u_adp_AlCu;

  AlCu_dmd_equations.evaluate_DV0_i_DF = evaluate_DV0_i_dF_bulk_AlCu;

  return AlCu_dmd_equations;
}
...
double evaluate_rho_i_adp_AlCu(unsigned int site_i,            //!
  ...
  for (unsigned int idx_j1 = 0; idx_j1 < numneigh_site_i; idx_j1++) {
    ...
    //! Create functions
    potential_function functions_rho_ij = rho_ij_adp_AlCu_constructor();
...
Eigen::Vector3d evaluate_DV_i_Dq_u_adp_AlCu(
    ...
    const AtomTopology atom_topology_i)  //!
{
  ...
  for (unsigned int idx_j1 = 0; idx_j1 < numneigh_site_i; idx_j1++) {
    ...
    //! Create functions
    potential_function functions_rho_ij = rho_ij_adp_AlCu_constructor();
    potential_function functions_pair_ij = V_pair_ij_adp_AlCu_constructor();
    ...
    for (unsigned int idx_j2 = idx_j1; idx_j2 < numneigh_site_i; idx_j2++) {
      ...
      potential_function functions_dipole_ij1j2 =
          V_dipole_ij1j2_adp_AlCu_constructor();
      potential_function functions_quadrupole_ij1j2 =
          V_quadrupole_ij1j2_adp_AlCu_constructor();
  ...
  CubicSpline embed_Al_ii = adp_AlAl.embed;
  CubicSpline embed_Cu_ii = adp_CuCu.embed;

  double d_F_embed_Al_i = xi_i * d_cubic_spline(&embed_Al_ii, rho_i);
  double d_F_embed_Cu_i = (1 - xi_i) * d_cubic_spline(&embed_Cu_ii, rho_i);

  D_V_i_embed_Dq = (d_F_embed_Al_i + d_F_embed_Cu_i) * D_rho_i_Dq;
...
Eigen::Matrix3d evaluate_D2V_i_Dq2_u_AlCu(
    unsigned int site_i_star,            //!
    unsigned int site_i,                 //!
    const Eigen::MatrixXd& mean_q,       //! Mean value of q
    const Eigen::VectorXd& xi,           //! Molar fraction
    ...
    const AtomTopology atom_topology_i)  //!
{
  ...
  for (unsigned int idx_j1 = 0; idx_j1 < numneigh_site_i; idx_j1++) {
    ...
    //! Define function
    potential_function functions_rho_ij = rho_ij_adp_AlCu_constructor();
    potential_function functions_V_pair_ij = V_pair_ij_adp_AlCu_constructor();
    ...
    for (unsigned int idx_j2 = idx_j1; idx_j2 < numneigh_site_i; idx_j2++) {
      ...
      //! Create functions
      potential_function V_dipole_ij1j2 = V_dipole_ij1j2_adp_AlCu_constructor();
  ...
  //! @brief Compute embedding hessian (to change by @jrecio1)
...
Eigen::Matrix3d evaluate_DV_i_dF_AlCu(
    ...
    const AtomTopology atom_topology_i)  //!
{
  ...
  //! @brief Compute embedding forces (to change by @jrecio1)
  CubicSpline embed_ii;
  if (spc_i == Mg) {
    embed_ii = adp_MgMg.embed;
  } else if (spc_i == H) {
...
#error "Define MULTIPOLE_INTEGRAL or GH3TH_INTEGRAL"
  ...
  for (unsigned int idx_j1 = 0; idx_j1 < numneigh_site_i; idx_j1++) {
    ...
    potential_function rho_ij = rho_ij_adp_AlCu_constructor();
...
#error "Define MULTIPOLE_INTEGRAL or GH3TH_INTEGRAL"
  ...
  for (unsigned int idx_j1 = 0; idx_j1 < numneigh_site_i; idx_j1++) {
    ...
    potential_function V_pair_ij = V_pair_ij_adp_AlCu_constructor();
  ...
  //! @brief Add the contribution of the embedded forces (to change by @jrecio1)
  CubicSpline embed_ii;
  if (spc_i == Mg) {
    embed_ii = adp_MgMg.embed;
  } else if (spc_i == H) {
...
Eigen::Vector3d evaluate_DV0_i_Dmeanq_u_AlCu(
    unsigned int site_i_star,            //!
    unsigned int site_i,                 //!
    const Eigen::MatrixXd& mean_q,       //!
    const Eigen::VectorXd& stdv_q,       //!
...
Eigen::Matrix3d evaluate_D2V0_i_Dmeanq2_u_AlCu(
    unsigned int site_i_star,            //!
    unsigned int site_i,                 //!
    const Eigen::MatrixXd& mean_q,       //!
    const Eigen::VectorXd& stdv_q,       //!
...
double evaluate_DV0_i_Dstdvq_u_AlCu(unsigned int site_u,                 //!
                                    unsigned int site_i,                 //!
                                    const Eigen::MatrixXd& mean_q,       //!
                                    const Eigen::VectorXd& stdv_q,       //!
                                    const Eigen::VectorXd& xi,           //!
...
#error "Define MULTIPOLE_INTEGRAL or GH3TH_INTEGRAL"
  ...
  //! @brief Compute the gradient of the embedding term (to change by @jrecio1)
  CubicSpline embed_ii;
  if (spc_i == Mg) {
    embed_ii = adp_MgMg.embed;
  } else if (spc_i == H) {
...
Eigen::Vector4d evaluate_DV0_i_Dq_u_AlCu(
    unsigned int site_i_star,            //!
    unsigned int site_i,                 //!
    const Eigen::MatrixXd& mean_q,       //! Mean value of q
...
double evaluate_DV0_i_Dxi_u_adp_AlCu(unsigned int site_u,                 //!
                                     unsigned int site_i,                 //!
                                     const Eigen::MatrixXd& mean_q,       //!
                                     const Eigen::VectorXd& stdv_q,       //!
                                     const Eigen::VectorXd& xi,           //!
...
#error "Define MULTIPOLE_INTEGRAL or GH3TH_INTEGRAL"
  ...
  for (unsigned int idx_j1 = 0; idx_j1 < numneigh_site_i; idx_j1++) {
    ...
    //! Define function
    potential_function f_rho_ij = rho_ij_adp_AlCu_constructor();
    potential_function V_pair_ij = V_pair_ij_adp_AlCu_constructor();
...
Eigen::Matrix3d evaluate_DV0_i_dF_bulk_AlCu(
    unsigned int site_i,                 //!
    const Eigen::MatrixXd& mean_q,       //!
